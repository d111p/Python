...
'''
The index of the book requested by the user is sent as a parameter together with the contents of the file in this module.

The book's size is then decreased, and a boolean with new contents is deleted.

It also determines whether or not the book specified by the user is available for borrowing.

If the book is available for borrowing, it advances to the book borrowing procedure; otherwise, an appropriate notice is displayed.
        ...
        try:   
            ...
            try:
                if(int(ListSplit.quantity[a])>0):
                    ...
                    count=1
                    while loop==True:
                        choice=str(input("Do you want to borrow more books? However you cannot borrow the same book twice. Press y for yes and n for no."))
                        if(choice.upper()=="Y"):
                            count=count+1
